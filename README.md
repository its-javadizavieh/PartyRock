# PartRock

PDF exports of the PartyRock course materials.

## Contents

- **AI, Cloud e PartyRock.pdf** — Slide deck (course content)
- **Prompt Kit — PartyRock.pdf** — Prompt templates for labs/activities

## 👨‍💻 Author

**Seyedhossein Javadizavieh**

📧 seyedhossein.javadizavieh@its-ictpiemonte.it

🔗 [LinkedIn Profile](https://www.linkedin.com/in/seyedhosseinjavadizavieh)

## Notes

These are PDF exports intended for easy sharing and printing.
